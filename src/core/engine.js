...
const REF = {
  ...
  sn:"RP 1707b Sec.3 (.SN/ continuation)",
...
function validateDotElements(rawLine, n, fromIdx, add, opts){
  ...
  hits.forEach((h,k)=>{
    ...
    // .RN/ and .SN/ are not just another element a space would fix: each
    // stands for a line break the 64-character limit forced on .R/ or .S/
    // respectively, so it can only ever be its own physical line. Telling
    // the operator to add a space would be telling them to do the one
    // thing that still leaves it wrong.
    if(h.tag==="RN"||h.tag==="SN")
      add(n,h.idx+1,1,"err",`.${h.tag}/ found glued to <b>${prev}</b> — a continuation must start its own line, not follow another element on the same one.`,h.tag==="RN"?REF.rn:REF.sn);
    else
