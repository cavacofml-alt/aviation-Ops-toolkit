...
/* Whether a bay is filled by an L/R pair or by one container on its own is a
   property of the aircraft/compartment, not of the ULD type — a leased
   aircraft's structure can pair up a type (e.g. LD8/DQF) that this operator's
   own fleet always loads whole-bay. The "+ L/R pair" form and the L<->R
   auto-mirror are therefore offered for every type; this list only supplies
   the placeholder hint shown in the single-position field (LD3 family and
   LD2 hint a bare bay number, matching how those types are usually typed). */
...
function groupBox(comp,g,gi){
  ...
  var addBtn = '<button class="btn small" data-act="open-pair" data-g="'+gi+'" style="border-color:'+gc+';color:'+gc+'"'+
...
function viewStep3(){
  ...
  var head = '<div style="display:flex;gap:10px;margin-bottom:16px;flex-wrap:wrap">'+
    ...
    (U.layouts && !blocked ? '<button class="btn primary" data-act="xlsx-selected">&#8595; Export selected configuration</button>':'')+'</div>' + mergeBox + gate+
    ...
    return '<div class="issue-kind">'+
      ...
      '<div class="issue-chips">'+
        shown.map(function(it){
          // a bulk hold, or anything the editor has no field for, is a label
          // rather than a button: there is nowhere for it to jump to
          var body = esc(it.name)+'<i>C'+esc(it.comp)+' &middot; '+esc(it.type)+'</i>';
          if(it.gid == null || it.ci == null)
            return '<span class="issue-chip" style="cursor:default" title="'+esc(it.reason)+'">'+body+'</span>';
          return '<button class="issue-chip" data-act="fix-pos" data-c="'+it.ci+'" '+
            'data-gid="'+esc(it.gid)+'" title="'+esc(it.reason)+'">'+body+'</button>';
        }).join("")+
...
/* Compartments that have candidate layouts but no layout marked as used —
   the final-configuration export needs one concrete choice per compartment,
   not "whichever one, pick any". */
function missingSelections(){
  return U.compartments.filter(function(c){
    return (U.layouts[c.number]||[]).length && U.selectedLayout[c.number] == null;
  }).map(function(c){ return c.number; });
}

/* Real conflicts between the layouts actually marked as used, compartment
   by compartment — the same overlap/lock test as conflictsWithSelection,
   just run once over every selected pair instead of one layout at a time.
   This is what the final-configuration export refuses to ship: unlike the
   full catalogue (every candidate, warnings only), this file is meant to be
   the one thing downstream loads without knowing any of this tool's rules —
   so it can never contain two selections that cannot coexist. */
function selectedConfigConflicts(){
  var nums = U.compartments.map(function(c){ return c.number; });
  var conflicts = [];
  for(var i=0; i<nums.length; i++){
    for(var j=i+1; j<nums.length; j++){
      var n1=nums[i], n2=nums[j];
      var i1=U.selectedLayout[n1], i2=U.selectedLayout[n2];
      if(i1==null || i2==null) continue;
      var l1=(U.layouts[n1]||[])[i1], l2=(U.layouts[n2]||[])[i2];
      if(!l1 || !l2) continue;
      var names1=l1.positions.map(function(p){return p.name;}), names2=l2.positions.map(function(p){return p.name;});
      var fwds1=l1.positions.map(function(p){return parseFloat(p.fwd);}), afts1=l1.positions.map(function(p){return parseFloat(p.aft);});
      var fwds2=l2.positions.map(function(p){return parseFloat(p.fwd);}), afts2=l2.positions.map(function(p){return parseFloat(p.aft);});
      var f1=Math.min.apply(null,fwds1), a1=Math.max.apply(null,afts1);
      var f2=Math.min.apply(null,fwds2), a2=Math.max.apply(null,afts2);
      var overlap = f1<a2 && f2<a1;
      var lockClash = (l1.locks||[]).some(function(nm){ return names2.indexOf(nm)>=0; })
                   || (l2.locks||[]).some(function(nm){ return names1.indexOf(nm)>=0; });
      if(overlap || lockClash) conflicts.push({ n1:n1, n2:n2, name1:l1.name, name2:l2.name, kind: lockClash?"lock":"overlap" });
    }
  }
  return conflicts;
}

// One row per position of the layout actually marked as used in each
// compartment — never the full catalogue. This is the file meant to leave
// the tool and be trusted as-is: it cannot be built at all while a
// compartment has no selection, or while two selections conflict (see
// selectedConfigConflicts) — there is no "export anyway" for either, unlike
// the reference export above.
function selectedConfigRows(){
  var rows = [];
  U.compartments.forEach(function(c){
    var idx = U.selectedLayout[c.number];
    var layout = idx == null ? null : (U.layouts[c.number]||[])[idx];
    if(!layout) return;
    layout.positions.forEach(function(pos){
      var seenTypes = {}, certTypes = (pos.certified||[{type:pos.uldType}])
        .filter(function(cc){ if(seenTypes[cc.type]) return false; return seenTypes[cc.type]=true; })
        .map(function(cc){ return cc.type+",LA"; }).join(";");
      rows.push([c.number, layout.name, pos.name, certTypes, +pos.fwd, +pos.aft,
        (pos.left===""||pos.left==null)?0:+pos.left,
        (pos.right===""||pos.right==null)?0:+pos.right,
        exportIndex(pos.index), 0, +pos.maxWeight]);
    });
  });
  return rows.concat(bulkRows());
}

/* ---------- the check before the file leaves the tool ----------
   The gate before generation reads the editor; this reads what the file is
   actually going to say, which is not the same thing. An index is rounded
   on the way out, a layout can have been generated before a position was
   edited, and the sign warning can have been acknowledged once and never
   asked about again. Everything downstream trusts this file to place weight
   on an aircraft, so it is worth one last look at the numbers as written.

   One position, not one row: a bay appears in every layout it fits, and
   that is the same bay to check a hundred times over. */
function exportedPositions(compNum){
  var seen = {}, out = [];
  U.compartments.forEach(function(c){
    if(compNum !== undefined && c.number !== compNum) return;
    ((U.layouts||{})[c.number]||[]).forEach(function(l){
      l.positions.forEach(function(p){
        var k = c.number+"|"+p.name+"|"+p.uldType;
        if(seen[k]) return;
        seen[k] = 1;
        out.push({ comp:c.number, p:p });
      });
    });
  });
  // bulk holds ride along in the combined export only
  if(compNum === undefined) (U.bulk||[]).forEach(function(h){
    (h.positions||[]).forEach(function(p){ out.push({ comp:h.number, p:p, bulk:true }); });
  });
  return out;
}
/* Which group a written position came from, so its chip can still jump to
   the field that produced it. */
function groupBehind(compNum, pos){
  var found = { ci:null, gid:null };
  U.compartments.forEach(function(c, i){
    if(c.number !== compNum) return;
    (c.uldGroups||[]).forEach(function(g){
      if(g.uldType !== pos.uldType) return;
      if((g.positions||[]).some(function(q){ return q.name === pos.name; })){ found = { ci:i, gid:g.id }; }
    });
  });
  return found;
}
/* A weight this far from what the rest of its type carries is a typing slip,
   not a derating: a 1587 kg LD3 keyed as 15870, or as 158. */
var EXPORT_WEIGHT_FACTOR = 3;
var EXPORT_WEIGHT_MIN_SAMPLE = 4;
function medianOf(nums){
  var s = nums.slice().sort(function(a,b){ return a-b; });
  return s.length ? s[Math.floor(s.length/2)] : null;
}
function exportIssues(compNum){
  if(!U.layouts) return [];
  var items = exportedPositions(compNum), out = [];

  // what most positions of each type carry, for the "unlike the others" check
  var byType = {}, typeMid = {};
  items.forEach(function(it){
    if(it.bulk) return;
    var w = parseFloat(it.p.maxWeight);
    if(!isNaN(w) && w > 0) (byType[it.p.uldType] = byType[it.p.uldType] || []).push(w);
  });
  Object.keys(byType).forEach(function(t){
    if(byType[t].length >= EXPORT_WEIGHT_MIN_SAMPLE) typeMid[t] = medianOf(byType[t]);
  });
  var idxLimit = {};
  U.compartments.forEach(function(c){ idxLimit[c.number] = grossIndexLimit(c); });

  items.forEach(function(it){
    var p = it.p;
    var g = it.bulk ? { ci:null, gid:null } : groupBehind(it.comp, p);
    var where = { comp:it.comp, ci:g.ci, gid:g.gid, name:p.name||"(unnamed)",
                  type:it.bulk ? "BULK" : p.uldType };
    var add = function(kind, reason){ out.push(Object.assign({kind:kind, reason:reason}, where)); };

    var raw = parseFloat(p.index), written = exportIndex(p.index);
    // 1. the sign, against the reference station — asked again here because
    //    the gate before generation can have been acknowledged once
    var sign = validateIndex(p.index, p.fwd, U.refStation);
    if(sign) add("Index sign against the reference station", sign+" (station "+p.fwd+", ref "+(U.refStation||"—")+")");
    // 2. an index small enough to round away carries no arm at all once written
    if(!isNaN(raw) && raw !== 0 && written === 0)
      add("Index rounds away to zero", "index "+p.index+" is written as 0 — the position would carry no moment arm");
    // 3. the weight the file authorises
    var w = parseFloat(p.maxWeight);
    if(isNaN(w) || w <= 0){
      add("Max weight is not usable", "max weight \""+p.maxWeight+"\" — nothing can be loaded against it");
    } else {
      if(w > GROSS_MAX_WEIGHT) add("Heavier than any ULD", "max weight of "+w+" kg");
      if(!it.bulk){
        var lightest = null;
        (p.certified||[]).forEach(function(cert){
          (U.ulds||[]).forEach(function(u){
            if(u.uldType !== cert.type || u.iata !== cert.iata) return;
            var m = parseFloat(u.maxWeight);
            if(!isNaN(m) && (lightest === null || m < lightest.mw)) lightest = { mw:m, iata:u.iata };
          });
        });
        if(lightest && w > lightest.mw)
          add("Above the ULD's own rating", w+" kg written against the "+lightest.iata+"'s own "+lightest.mw+" kg");
        var mid = typeMid[p.uldType];
        if(mid && (w > mid*EXPORT_WEIGHT_FACTOR || w < mid/EXPORT_WEIGHT_FACTOR))
          add("Weight unlike the rest of its type", w+" kg where most "+p.uldType+" positions carry about "+mid+" kg");
      }
    }
    // 4. an index orders of magnitude away from the rest of its hold
    var lim = idxLimit[it.comp];
    if(lim && !isNaN(raw) && Math.abs(raw) > lim)
      add("Index far outside the compartment", "index of "+p.index+" — check the decimal point");
  });

  // the file would describe numbers the editor no longer holds
  if(U.layoutsStale) out.unshift({ kind:"Generated before the last edit", comp:"—", ci:null, gid:null,
    name:"all", type:"—",
    reason:"a position was edited after these layouts were computed" });
  return out;
}
...
function onUldClick(e){
  ...
  else if(act==="xlsx-selected"){
    var missing = missingSelections();
    if(missing.length){ selectionRequiredModal(missing); return; }
    var confl = selectedConfigConflicts();
    if(confl.length){ selectionConflictModal(confl); return; }
    exportGuard(undefined, true, function(){
      downloadXlsx("D3", EXPORT_HEADERS, selectedConfigRows(), "selected_configuration.xlsx"); });
  }
}
...
function exportGuard(compNum, requireBulk, run){
  ...
  host.innerHTML = '<div class="modal-back"><div class="modal">'+
    '<div class="mh"><b>&#9888; Check before exporting</b>'+
      '<button class="btn small quiet" data-x="close">Close</button></div>'+
    '<div class="mb">'+
      '<p class="note" style="margin-top:0">These are the numbers as the file will carry them — '+
      'rounded to the five decimals the upload takes, and read back from the layouts themselves. '+
      'Everything downstream places weight on an aircraft from this file.</p>'+
      '<div class="issuebox"><div class="issue-body" style="padding-top:12px">'+ issueBody(issues) +'</div></div>'+
      '<div style="display:flex;gap:8px;margin-top:14px;flex-wrap:wrap;align-items:center">'+
        '<button class="btn small primary" data-x="go">&#8595; Export anyway</button>'+
        '<button class="btn small" data-x="close">Go back and check</button>'+
        '<span class="note">'+issues.length+' to look at</span>'+
      '</div>'+
    '</div></div></div>';
  host.addEventListener("click", function(e){
    var t = e.target;
    var chip = t.closest ? t.closest('[data-act="fix-pos"]') : null;
    if(chip){                       // straight to the field that produced it
      host.innerHTML = "";
      U.step = 1;
      U.activeComp = +chip.getAttribute("data-c");
      U.collapsed[chip.getAttribute("data-gid")] = false;
      uldRender();
      return;
    }
    var btn = t.closest ? t.closest("[data-x]") : null;
    if(!btn && t !== host.querySelector(".modal-back")) return;
...
/* No "export anyway": the whole point of this export is that the file
   leaving the tool never contains a compartment with no concrete choice —
   that ambiguity is exactly what the full catalogue export is for. */
function selectionRequiredModal(missing){
  var host = $("modalHost");
  host.innerHTML = '<div class="modal-back"><div class="modal">'+
    '<div class="mh"><b style="color:var(--red)">&#9888; Missing a selection</b>'+
      '<button class="btn small quiet" data-x="close">Close</button></div>'+
    '<div class="mb">'+
      '<p class="note" style="margin-top:0;color:var(--red)">Compartment'+(missing.length!==1?"s":"")+' '+
      esc(missing.join(", "))+' '+(missing.length!==1?"have":"has")+' candidate layouts but nothing marked as used yet. '+
      'This export only ever contains the layout you chose for each compartment — go to Layouts and mark one in each.</p>'+
      '<div style="display:flex;gap:8px;margin-top:6px;flex-wrap:wrap">'+
        '<button class="btn small primary" data-x="close">Go back and choose</button>'+
      '</div>'+
    '</div></div></div>';
  host.addEventListener("click", function(e){
    var btn = e.target.closest ? e.target.closest("[data-x]") : null;
...
/* Also no "export anyway": a file meant to be trusted as-is by whatever
   reads it downstream can never carry two selections that physically or by
   a fixed combination's lock cannot coexist — that is the one thing this
   export exists to guarantee that the full-catalogue export does not. */
function selectionConflictModal(conflicts){
  var host = $("modalHost");
  var rows = conflicts.map(function(cf){
    var why = cf.kind==="lock" ? "one keeps a position empty the other uses" : "same physical zone";
    return '<li>Compartment '+cf.n1+' "'+esc(cf.name1)+'" &harr; Compartment '+cf.n2+' "'+esc(cf.name2)+'" — '+why+'</li>';
  }).join("");
  host.innerHTML = '<div class="modal-back"><div class="modal">'+
    '<div class="mh"><b style="color:var(--red)">&#9888; Selections conflict with each other</b>'+
      '<button class="btn small quiet" data-x="close">Close</button></div>'+
    '<div class="mb">'+
      '<p class="note" style="margin-top:0;color:var(--red)">The layouts marked as used cannot coexist — this export '+
      'refuses to ship a configuration that could never actually be loaded. Change one of the selections below in Layouts.</p>'+
      '<ul style="margin:0;padding-left:18px;color:var(--dim)">'+ rows +'</ul>'+
      '<div style="display:flex;gap:8px;margin-top:14px;flex-wrap:wrap">'+
        '<button class="btn small primary" data-x="close">Go back and fix</button>'+
      '</div>'+
    '</div></div></div>';
  host.addEventListener("click", function(e){
    var btn = e.target.closest ? e.target.closest("[data-x]") : null;
