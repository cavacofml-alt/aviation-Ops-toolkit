...
function layoutConflicts(compNum, layout){
  ...
  U.compartments.forEach(function(c){
    ...
    (U.layouts[c.number]||[]).forEach(function(l2){
      var names2 = l2.positions.map(function(p){ return p.name; });
      var fwds2 = l2.positions.map(function(p){ return parseFloat(p.fwd); });
      var afts2 = l2.positions.map(function(p){ return parseFloat(p.aft); });
      var fwd2 = Math.min.apply(null, fwds2), aft2 = Math.max.apply(null, afts2);
      var lockClash = (layout.locks||[]).some(function(nm){ return names2.indexOf(nm)>=0; })
                   || (l2.locks||[]).some(function(nm){ return names.indexOf(nm)>=0; });
      var overlap = fwd < aft2 && fwd2 < aft;
      if(lockClash || overlap) out.push({ n2:c.number, name2:l2.name, kind: lockClash?"lock":"overlap" });
    });
  });
  return out;
}
...
function renderLayoutList(compNum, list){
  ...
  var html = shown.map(function(l, i){
    ...
    // Grouped by compartment and kind rather than named one by one — a busy
    // compartment can have hundreds of layouts that overlap this one, and a
    // wall of names is exactly the "big confusing number" this replaced.
    var byComp = {};
    conflicts.forEach(function(cf){
      var k = cf.n2+":"+cf.kind;
      if(!byComp[k]) byComp[k] = { n2:cf.n2, kind:cf.kind, count:0, example:cf.name2 };
      byComp[k].count++;
    });
    var conflictNote = conflicts.length
      ? '<div class="note" style="color:var(--amber);margin-top:8px">&#9888; Can\'t be used at the same time as: '+
          Object.keys(byComp).map(function(k){
            var g = byComp[k];
            var why = g.kind==="lock" ? "keeps a position empty this one uses" : "same physical zone";
            return g.count+' layout'+(g.count!==1?'s':'')+' in Compartment '+g.n2+' ('+why+', e.g. “'+esc(g.example)+'”)';
          }).join("; ")+
        '</div>'
      : '';
    var body = open
    ...
    return '<div class="layout-item">'+
      '<div class="layout-name" data-act="toggle-layout" data-k="'+esc(key)+'">'+
        ...
        (conflicts.length ? '<span style="color:var(--amber)" title="Can\'t be used with '+conflicts.length+' layout(s) in other compartments">&#9888;</span>' : '')+
