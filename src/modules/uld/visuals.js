...
function aircraftPanel(activeNum, mode){
  ...
  var blocks = comps.map(function(c,i){
    ...
    // Active hold: draw each K/L/P zone (11, 12, 13…) as its own tick inside
    // the box, at its real station width, instead of leaving the enlarged
    // box empty. The "A/M/N/Q" (P-row) string sits on a separate deck in
    // real aircraft, so it's left out of this single-row view — the
    // detailed zone grid below still covers it.
    var zonesHtml = "";
    if(on){
      var zoneBases = {};
      c.uldGroups.forEach(function(g){ g.positions.forEach(function(p){
        ...
        if(!zoneBases[base]) zoneBases[base] = {f:f,a:a};
        else { zoneBases[base].f = Math.min(zoneBases[base].f,f); zoneBases[base].a = Math.max(zoneBases[base].a,a); }
      }); });
      var zones = Object.keys(zoneBases).map(function(b){
        return {name:b, f:zoneBases[b].f, a:zoneBases[b].a}; }).sort(function(p,q){ return p.f-q.f; });
      var zy = y+18, zh = h-26;
      zonesHtml = zones.map(function(z){
        var zx1=st2x(z.f), zx2=st2x(z.a), zw=Math.max(2,zx2-zx1);
        return '<rect x="'+zx1.toFixed(1)+'" y="'+zy+'" width="'+zw.toFixed(1)+'" height="'+zh+'" '+
            'fill="'+col+'" fill-opacity="0.16" stroke="'+col+'" stroke-width="1" rx="1"></rect>'+
          '<text x="'+((zx1+zx2)/2).toFixed(1)+'" y="'+(zy+zh/2+4)+'" text-anchor="middle" '+
            'style="fill:'+col+'" font-size="9" font-family="monospace">'+esc(z.name)+'</text>';
      }).join("");
    }
    ...
    return '<g class="hold" data-act="'+act+'" data-i="'+idx+'" data-n="'+i+'" style="cursor:pointer">'+
      ...
      '<rect x="'+x1.toFixed(1)+'" y="'+y+'" width="'+Math.max(2,(x2-x1)).toFixed(1)+'" '+
        'height="'+h+'" fill="'+col+'" fill-opacity="'+(on?0.12:0.16)+'" '+
        'stroke="'+col+'" stroke-width="'+(on?2.5:1.3)+'" rx="2"></rect>'+
      (on
        ? '<text x="'+(x1+6).toFixed(1)+'" y="'+(y+13)+'" style="fill:'+col+'" font-size="12" '+
          'font-weight="700" font-family="monospace">'+c.number+'</text>'+ zonesHtml
